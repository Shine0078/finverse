...
describe('manual transactions on PostgreSQL', () => {
  ...
  beforeAll(async () => {
    if (!OWNER_URL) throw new Error('This persistence regression requires npm run test:db.');
    harness = await startPgHarness(OWNER_URL);
    Object.assign(process.env, { STORE: 'postgres', DATABASE_URL: OWNER_URL,
      DATABASE_APP_URL: appUrlFrom(OWNER_URL), MIGRATE_ON_BOOT: 'false', NODE_ENV: 'test',
